...
public class UserAdapter extends RecyclerView.Adapter {
    ...
    public class MyView extends RecyclerView.ViewHolder {
        ...
        @OnClick(R.id.llFinalLayout)
        public void onClick() {
            for (int i = 0; i < mlist.size(); i++) {
                if (i != getAdapterPosition()) {
                    mlist.get(i).setiSclicked(false);
                }
                mlist.get(getAdapterPosition()).setiSclicked(true);
                viewpager.setCurrentItem(getAdapterPosition(), true);
            }
            mAdapterCallback.onMethodCallback(mlist.get(getAdapterPosition()));
